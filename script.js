  ...
  fetch("emojis.json")
    ...
    .then((data) => {
      ...
      emojiSearch.addEventListener("input", function () {
        ...
        filteredEmojis = data.filter(
          (emoji) =>
            emoji.description && emoji.description.toLowerCase().includes(query)
        );
        // Limpiar los resultados anteriores
        renderEmojiResults(emojiResults, filteredEmojis);
      });
      renderEmojiResults(emojiResults, filteredEmojis);
    })
    .catch((error) => {
...
function renderEmojiResults(emojiResults, filteredEmojis) {
...
function copyCode(code) {
  navigator.clipboard
    ?.writeText(code) // Verificar si el navegador soporta el API Clipboard
    .then(() => {
